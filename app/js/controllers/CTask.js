(() => {
	...
	angular.module('app')
		.controller('CTask', function(FTask)
		{
			...
			const FILTER_ALL 			= '';
			const FILTER_CURRENT		= '!true';
			const FILTER_COMPLETED		= 'true';

			const NAV_ALL_INDEX			= 0;
			const NAV_CURRENT_INDEX 	= 1;
			const NAV_COMPLETED_INDEX	= 2;
			...
			vm.getTaskMessage = function()
			{
				...
				if( 	 vm.navigationItems[NAV_CURRENT_INDEX].count 	=== 1 												&&
						 vm.activeNavigation.name 						=== vm.navigationItems[NAV_ALL_INDEX].name 			||
						 vm.navigationItems[NAV_CURRENT_INDEX].count 	=== 1 												&&
						 vm.activeNavigation.name 						=== vm.navigationItems[NAV_CURRENT_INDEX].name)
				{
				...
				//		 > 1 Current task
				//	AND  All OR Current active navigation
				else if( vm.navigationItems[NAV_CURRENT_INDEX].count 	> 	1 												&&
						 vm.activeNavigation.name 						=== vm.navigationItems[NAV_ALL_INDEX].name 			||
						 vm.navigationItems[NAV_CURRENT_INDEX].count 	> 	1 												&&
						 vm.activeNavigation.name 						=== vm.navigationItems[NAV_CURRENT_INDEX].name)
				{
				...
				//	AND  >= 1 All task count
						 ...
						 vm.activeNavigation.name 						=== vm.navigationItems[NAV_ALL_INDEX].name 			&&
						 vm.navigationItems[NAV_ALL_INDEX].count 		>= 	1 												||
						 vm.navigationItems[NAV_CURRENT_INDEX].count 	< 	1 												&&
						 vm.activeNavigation.name 						=== vm.navigationItems[NAV_CURRENT_INDEX].name 		&&
						 vm.navigationItems[NAV_ALL_INDEX].count 		>= 	1)
				{
				...
				else if( vm.navigationItems[NAV_COMPLETED_INDEX].count 	=== 1 												&&
						 vm.activeNavigation.name 						=== vm.navigationItems[NAV_COMPLETED_INDEX].name)
				{
